...
One further concern is whether our implementation tells us when these constraints bind, or more generally, where in the state-space the 'cross-overs to $-\infty$' are occurring. Here the third possibility has a clear advantage over the fourth: if we ask the model solution whether many agents find the constraint binding (i.e., whether many agents make decisions that are up against the constraint), then the third (and second) approach will highlight that they do, while the fourth (and first) approach obscures this. A related observation is that the flags generated by the third (and fourth) approaches can themselves be used as a diagnostic, suggesting where in the state-space the grid would benefit from additional points.
