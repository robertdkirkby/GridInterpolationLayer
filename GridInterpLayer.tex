...
\usepackage{hyperref}
\hypersetup{
  pdftitle={Grid Interpolation Layer},    % title
...
\For{$j$ count backward from $N_j$ to $1$}
   ...
   \For{All values $z$}
    ...
    \State $\quad$ $\quad$ onto the entire fine grid to get $\hat{EV}_j(a,z)$.
     ...
     \State $\quad$ $\quad$ $aL2\_{}ind=(g^1_j(a,z)-2)*(n2+1)+(1:1:(3+2*n2))$  \Comment fine-grid indices for the second-layer band; given $(a,z)$
     \State $\quad$ $\quad$ $aL2\_{}grid=linspace(g^1_j(a,z)-1,g^1_j(a,z)+1,3+2*n2)$  \Comment Not used, illustrates what $aL2\_{}ind$ corresponds to when it indexes the fine grid.
...
One thing worth discussing is what form the optimal policy $[g_1,g_2,...,g_{N_j}]$ should be stored in. I choose to store it as the 'index of the lower grid point of $a\_{}grid$' (which is a number from 1 to $n-1$) together with the index of the second layer (which is a number from 1 to $n2+2$). When using the policy to create the agent distribution we will want the 'upper and lower grid points of $a\_{}grid$' (the upper is just lower plus 1, so that is easy) and the 'weights' which can be trivially calculated from the second layer index (weight of lower grid point is $1-(\textit{second layer index}-1)/(n2+1)$; because we placed the second layer points evenly spaced). When using the policy to evaluate fns on the agent dist we want the aprime value, which we can trivially do by creating the 'fine' grid, $aprime\_{}grid=interp1(1:1:n,a\_{}grid,linspace(1,n,n+n2*(n-1)))$ and getting the index for the fine grid as 'fine grid index'=$(n2+1)*($'index of the lower grid point of $a\_{}grid$'$ -1)+$'second layer index', then simply use the index for the fine grid to get the grid value from the fine grid. Since we created the indexes while solving the value fn problem,\footnote{When we solve the first layer we got the 'midpoint', switching this to the 'lower grid point' just requires subtracting one from it.} it makes sense to store these, and only get the weights as needed for the agent dist.
...
Explicitly, the finite horizon problem with one endogenous state, one decision variable, and one markov shock is solved as follows. This is the same as the pseudocode above, but with the decision variable $d$ added.
\begin{algorithmic}
\State{Create the fine grid: $afine\_{}grid=interp1(1:1:n, a\_{}grid, linspace(1,n, n+(n-1)*n2))$.}
\For{$j$ count backward from $N_j$ to $1$}
   \State Get $\theta_j$ from $\Theta$
   \For{All values $z$}
    \State Calculate $EV_j(a',z)=E[V_{j+1}(a',z')]$ \Comment For $j=N_j$ this is zero
    \State Interpolate $EV_j(a,z)$ from the first layer grid onto the
    \State $\quad$ $\quad$ entire fine grid to get $\hat{EV}_j(a,z)$.
     \For{All values of $(a,z)$}
     \State \textbf{First Layer} (done conditional on each $d$)
     \State For each $d$, solve for optimal $aprime$ on grid:
     \State $\quad$ $\quad$ $g^1_{j}(d,a,z)=argmax_{i=1,...,n} F(d,a\_{}grid(i),a,z) + \beta EV_j(a\_{}grid(i),z)$
     \State \textbf{Second Layer} (a different band for each $d$)
     \State For each $d$, create the second layer grid around its midpoint:
     \State $\quad$ $\quad$ $aL2\_{}ind(d)=(g^1_j(d,a,z)-2)*(n2+1)+(1:1:(3+2*n2))$
     \State Jointly solve for optimal $(d,aprime)$ on the second layer:
     \State $\quad$ $\quad$ $(d^*_j(a,z),g^2_j(a,z))=$
     \State $\quad$ $\quad$ $\quad$ $argmax_{d,\;i=1,...,3+2*n2} F(d,afine\_{}grid(aL2\_{}ind(d,i)),a,z) + \beta \hat{EV}_j(aL2\_{}ind(d,i),z)$
    \EndFor
   \EndFor
  \EndFor
  \State Convert $(g^1_j(d^*_j(a,z),a,z),\, g^2_j(a,z))$ into $(lower\_{}index, second\_{}layer\_{}index)$:
  \State $\quad$ $\quad$ if $g^2_j(a,z) \leq n2+1$: $lower\_{}index=g^1_j(d^*_j(a,z),a,z)-1$, $second\_{}layer\_{}index=g^2_j(a,z)$
  \State $\quad$ $\quad$ else: $lower\_{}index=g^1_j(d^*_j(a,z),a,z)$, $second\_{}layer\_{}index=g^2_j(a,z)-(n2+1)$
  \State Store the policy as $g_j(a,z)=[d^*_j(a,z),\;lower\_{}index,\;second\_{}layer\_{}index]$.
  \\ \Return $[V_1,...,V_{N_j}]$, $[g_1,...,g_{N_j}]$
\end{algorithmic}
Relative to the no-decision-variable pseudocode, the only changes are that the first layer optimum $g^1_j$ is now found separately for each $d$, the second layer builds a separate band around the midpoint of each $d$, and the second layer argmax is taken jointly over $d$ and the second layer index to give the optimal $(d^*_j,g^2_j)$. The midpoint edge-handling (add/subtract one when $g^1_j$ is the first or last grid point) is exactly as for the no-$d$ case above. This is exactly how the Matlab code in \texttt{ValueFnIter\_{}FHorz\_{}GI1\_{}raw.m} is organized.
...
Everything described above could easily be done without the second layer grids being evenly spaced, and by using any interpolation other than linear. The evenly spaced grids are intuitive and easy, and it is not obvious that there is an alternative choice that would meaningfully improve performance (the choice of a good $a\_{}grid$ is likely much more important, as are the choice of $n$ and $n2$). As for linear interpolation, you could easily use something else. I have not tested the alternatives: they will increase runtime and improve accuracy, but will the improved accuracy be worth the increased runtime? The code in VFI Toolkit does the interpolation using Matlab's 'interp1()', so if anyone does want to test them it is as trivial as changing \textit{interp1(a\_{}grid,EV,aprime\_{}grid)} to \textit{interp1(a\_{}grid,EV,aprime\_{}grid,'spline')}, or similar (note, not all the interpolation methods in \textit{interp1()} work with GPU).
...
\caption{The case where the return function takes the value $-\infty$ at $R_2$. Unlike the value function case, the intermediate return function values $R_{2.1},\dots,R_{2.5}$ can still be evaluated exactly. But the question is what to do here?}
\label{fig:rinf}
\end{figure}
...
Fundamentally, what we are seeing here is that there is no easy way to deal with interpolation when you have a finite number on one side and a $-\infty$ on the other. Evaluating the return function exactly at the intermediate points at first glance allows us to side-step this, but looking closely we see that actually this just shifts the problem into the simulation/interpolation where we linearly interpolate back onto the grid.

In the end, possibility three has been implemented in VFI Toolkit. We get the most accurate grid interpolation point in the value and policy functions, and we add a flag that sets the weight to zero during simulation/iteration for any point that would involve implicitly choosing a $-\infty$ in the return function. The two main motives for this decision were: (i) the Policy still shows agents choosing to go right up against the constraints, which is useful for users who want to check whether the constraints bind; (ii) no agent in the agent distribution ever ends up at a point that could only be reached by violating the constraints. The weakness is, of course, the small difference between the probabilities/weights used to construct the value function and those used to iterate the agent distribution. But, as with all the possibilities, this difference shrinks to irrelevance as the coarse grid is increased in size, shrinking the spacing between coarse grid points.
...
More precisely, the flag is set immediately after the second layer argmax in the pseudocode in the Pseudocode subsection above, between the line that computes $g^2_{j}(a,z)$ and the conversion to $(lower\_{}index, second\_{}layer\_{}index)$, by inserting:
\begin{algorithmic}
\State Set the L2 flag (handles $F=-\infty$ at a coarse band endpoint):
\State $\quad$ $\quad$ if $g^2_{j}(a,z)\in\{2,\dots,n2+1\}$ and $F(afine\_{}grid(aL2\_{}ind(1)),a,z)=-\infty$: $L2flag=3$
\State $\quad$ $\quad$ elseif $g^2_{j}(a,z)\in\{n2+3,\dots,2*n2+2\}$ and $F(afine\_{}grid(aL2\_{}ind(3+2*n2)),a,z)=-\infty$: $L2flag=1$
\State $\quad$ $\quad$ else: $L2flag=2$
\end{algorithmic}
\noindent Here $aL2\_{}ind(1)$ and $aL2\_{}ind(3+2*n2)$ are the fine-grid indices of the lower and upper coarse endpoints of the band, i.e.\ coarse points $g^1_{j}(a,z)-1$ and $g^1_{j}(a,z)+1$. The conditions on $g^2_{j}(a,z)$ pick out the cases in which the second layer optimum lies strictly inside one of the two sub-intervals that make up the band.

When the stored second layer index is converted into transition weights for iterating the agent distribution, the flag overrides the standard linear-interpolation formula as follows. If $L2flag=1$, the second layer index is treated as $1$, so all mass goes onto the lower first layer grid point. If $L2flag=3$, the second layer index is treated as $n2+2$, so all mass goes onto the upper first layer grid point. If $L2flag=2$, the stored second layer index is used unchanged. In all three cases the agent distribution iteration then applies the standard formula: the weight on the lower first layer grid point is $1-(\textit{second layer index}-1)/(n2+1)$ and the weight on the upper first layer grid point is $(\textit{second layer index}-1)/(n2+1)$.
